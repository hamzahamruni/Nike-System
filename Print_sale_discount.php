...
<?
	if(isset($_POST['Cash_Sale']))
	{
		...
		{
			if($_POST['total_p']>0 AND $_POST['total_p']<9000)
			{
				if($_POST['total_p']!=$total_price->total_price)
				{
					$sql_count="SELECT COUNT(`id_portfolio`) AS count_class  FROM portfolio";
					$count=$conn->query($sql_count);
					$count_class=$count->fetch(PDO::FETCH_OBJ);	
					$discount_class=($total_price->total_price-$_POST['total_p'])/$count_class->count_class;
				}
				$total_price->total_price=$_POST['total_p'];
			}
...
?>
			...
			<?
						...
						{	
							$View_Class->Price-=$discount_class;
...
//									Insert 
									...
									$sql_update="UPDATE ClasS SET del='1' WHERE Id_class='$View_Class->Id_class' ";

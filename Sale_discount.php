...
function valid()
{
	...
	if(document.getElementById("barcode").value == "" && document.getElementById("name").value == "")
	{
...
//					search
			...
			if($Barcode!="")
			{
				$sql1="SELECT * FROM Class  WHERE  barcode='$Barcode' AND del='0'
					AND 
					Id_class  NOT
					IN 
					(
			...
			else
			{
				if($name!="")
				{
					$sql1="SELECT * FROM Class WHERE  Name='$name' AND del='0'
					AND 
					Id_class  NOT
					IN 
					(
			...
			{
				$sql2="INSERT INTO portfolio (id_portfolio,price) VALUES ('$Sale_Class->Id_class','$Sale_Class->Price')";
				if($conn->exec($sql2))
				{
						...
						<div class="content">
							<div class="module">
								<div class="module-head"><ul class="widget widget-menu unstyled">
									...
									<form action="Print_sale_discount.php"  method="post"  onsubmit="return End_Bill();">

											<button type="submit" name="Cash_Sale" id="Print" class="btn btn-primary pull-right">بيع و طباعة <i class="icon-money"></i></button>
										
									
									
									<li><center><h2><? echo " إجمالي السعر  : <input type='text' name='total_p' id='total_p' maxlength=4  class='span2'  placeholder='السعر بعد التخفيض'>
 ",$total_price->total_price," د.ل   "; ?></h2></center>

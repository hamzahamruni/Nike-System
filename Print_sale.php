...
<?
	if(isset($_POST['Cash_Sale']))
	{
		...
		if(!(isset($paid)))
		{
			$paid=$total_price->total_price;
		}
		
		
							$id_user=$_SESSION['id'];
							$sql_insert_sales="INSERT INTO sale (Id_sale,date,total_price,paid,id_user,id_account,del) VALUES (null,CURRENT_TIMESTAMP,'$total_price->total_price','$paid','$id_user','$id_account','0')";
...
?>
			...
			<?
						$sql3="SELECT * FROM Class a,purchases p,portfolio po 
									WHERE
									p.id_class=a.Id_class
									AND
									po.id_portfolio=a.Id_class
									AND
									p.qty_store > '0' AND p.qty_store>=po.qty_por
						...
						{
							$sql_insert_sales_realtion="INSERT INTO relation_class_sale (id_relation,id_sale,id_class,qty_sale,class_price) VALUES (null,'$Id_sale','$View_Class->Id_class','$View_Class->qty_por','$View_Class->price')";
...
//									Insert 
									...
									$sql_update="UPDATE purchases SET qty_store=qty_store-'$View_Class->qty_por' WHERE id_class='$View_Class->Id_class' ";
								...
								echo"<tr>
									  ...
									  <td>$View_Class->qty_por</td>
									  <td>$View_Class->price د.ل</td>
									  <td>",$View_Class->price*$View_Class->qty_por," د.ل</td>
									</tr>";
								}
							}
							?>
									...
									<table class="table table-bordered">
										...
										<tr>
												<th>
										<center>	<h3> القيمة المدفوعة  <? echo "$paid ";?> د.ل </h3></center>
											</th>
										</tr>
											
											<?
										if($total_price->total_price-$paid>0)
										{
											?>
										<tr>
											<th>
										<center>	<h3> الباقي <? echo $total_price->total_price-$paid;?> د.ل </h3></center>
											</th>
										</tr>
										<?
										}

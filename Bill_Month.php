...
function form($conn)
{
						...
						$sql_search_Bill_Data="SELECT * FROM  sale WHERE date>='$year-01-01 00:00:01' AND date<='$year-12-31 23:59:591' AND del='0'";
						...
						<div class="content">
							...
							<div class="module">
								<div class="module-head">
									<h3>تقرير إردات الشهر  <i class="icon-print"></i></h3>
									<hr>
								...
								<div class="control-group">			
									<div class="controls">
											...
											{
												if(date('m',strtotime($Data_Bill->date))!=$month)
												{
													$month=date('m',strtotime($Data_Bill->date));
											
											?>
														<option value="<?php echo date('m',strtotime($Data_Bill->date)); ?>"><?php echo date('m F',strtotime($Data_Bill->date)); ?></option>
											<?
												}
											}
											?>
...
function View($conn)
{
		...
		$sql_search_Bill_Data="SELECT * FROM sale s,relation_class_sale r,class c WHERE r.id_sale=s.Id_sale AND r.id_class=c.Id_class AND s.del='0' AND ( date>='$year-$month-01 00:00:01' AND date<='$year-$month-31 23:59:59') ORDER BY date,s.Id_sale  ";
					...
					<div class="content">
						<div class="module">
								<div class="module-head"><ul class="widget widget-menu unstyled">
									<center><h2> إردات الشهر <br><? $Time=mktime(00,00,00,$month,gmdate("d", time() + 2 * 3600),$year); echo date(" Y-m F",$Time); ?> </h2></center>
								</div>						
							<div class="module-body">
								...
								if($Search_Data_Bill)
            					{
								 ...
								 <?
											$Total_Buy_Month+=$Data_Bill->Price_buy;
										}
								...
								<br>
									<table class="table table-bordered">
										...
										<tr>
											<th>
											<center>
											<h3> إجمالي إردات الشهر <? echo "$Total_Sales_Day د.ل";?></h3>
											<?
											if($_SESSION['Priv']=='A')
											{
												?>
												<h3> إجمالي الشراء  <? echo "$Total_Buy_Month د.ل";?></h3>
												<h3> إجمالي الربح <? echo $Total_Sales_Day-$Total_Buy_Month," د.ل";?></h3>
												<?
											}
											?>
											</center>
									...
									<br>
								<form action="Print_Bill_Month.php" target="_blank" method="post">

								...
								<form action="" method="post" class="form-horizontal row-fluid" onsubmit="return valid();">
										...
										?>
											<div class="control-group">
												...
												<div class="controls">
												<?
												echo "SELECT * FROM user_accounts u WHERE u.del='0' order by u.account_char,u.account_name ASC ";
												$Search_account=$conn->query("SELECT * FROM user_accounts u WHERE u.del='0' order by u.account_char,u.account_name ASC ");
															...
															while($view_account=$Search_account->fetch(PDO::FETCH_OBJ))
															{
																...
																if($view_account->account_char=='C' )
																{
																	...
																	if($C)
																	{
																		$C=false
																	?>
																	<optgroup label="العميل">
																	<?
																	}
																	?>
																	
																	<option value="<?echo $view_account->id_account;?>" >
																	<?echo $view_account->account_name," ";?>
																	</option>
										...
										?>	
										<input hidden type="text" name="id_account" value="<?php echo $id_account	; ?>"/>
										<?
							...
							<div class="module">
								<div class="module-head"><ul class="widget widget-menu unstyled">
									...
									<form action="Print_sale.php"  method="post"  onsubmit="return End_Bill();">
										<input hidden type="text" name="id_account" value="<?php echo $id_account	; ?>"/>
										<input hidden type="number" id="total_price" name="total_price" value="<?php echo $total_price->total_price; ?>"/>

										<button type="submit" name="Cash_Sale" id="Print" class="btn btn-primary pull-right"><i class="icon-money"></i> بيع و طباعة </button>
										
										<center>
										<label class="control-label" id="lpaid"  >القيمة المدفوعة</label>
											<div class="controls">
												...
												{
													?>
													<input type="number" name="paid" id="paid" maxlength=5 value="<? echo $_POST['paid']	;?>"  maxlength=4 class="span2"><span class="add-on">دينار</span>
													<?
												}
												else
												{
													?>
													<input type="number" name="paid" id="paid" maxlength=5 class="span2"><span class="add-on">دينار</span>
													<?
												}
												?>
													<label id="paidmsg"></label>	
												</div>
											</div> 
										</center>
